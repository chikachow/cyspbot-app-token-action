...
- `cyspbot-token-url`
  - optional exact credential-free HTTPS token exchange endpoint URL override
...
The action requests a GitHub Actions OIDC token for cyspbot's fixed service audience, `https://cyspbot.chikachow.org`. It derives the default token endpoint by appending `/token` to that audience. An explicit `cyspbot-token-url` changes only the request destination; it does not change the OIDC audience. The URL must be canonical, HTTPS, credential-free, and contain no query or fragment. Broker redirects are rejected, and the returned scope must exactly equal the requested scope. The action does not send an RFC 8693 token-exchange `audience` form field; cyspbot uses `resource` as the issued-token target.
