...
- The action exposes `token`, `expires_at`, and `scope` outputs and accepts `cyspbot-token-url`, `resource`, and `scope` inputs.
- The action requests a GitHub Actions OIDC token for cyspbot's fixed `cyspbot` audience. It does not send an RFC 8693 token-exchange `audience` form field because cyspbot uses `resource` as the issued-token target.
